...
- `authCookie` 与 API key 是敏感凭据，插件仅用于向对应官方服务请求用量/余额数据，不会写入日志或发送到其他任何地方
- `opencode-go-usagebar.json` sidecar 文件含凭据，**不要提交到 git**（仓库的 `.gitignore` 已排除常见凭据文件）
- 环境变量方式（`OPENCODE_GO_USAGEBAR_*`）比 sidecar 文件更安全，建议优先使用
...
发布使用 npm 的 OIDC trusted publishing（无需长期 token），并在 GitHub 自动生成 Release 与 provenance 来源证明。npm 侧需在包的 Settings → Trusted publishing 配置与仓库 `release.yml` 匹配的 trusted publisher。
